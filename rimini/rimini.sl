% rimini.sl
% 
% $Id: rimini.sl,v 1.1.1.1 2004/10/28 08:16:25 milde Exp $
...
% switch_to_buffer is an intrinsic, apparently LAST_KBD_COMMAND doesn't
% work for those.  And which_key does some substitutions that are too much
% work to undo.
custom_variable ("switch_buffer_binding", "b");
...
public define mini_isearch ()
{
   ...
   if (rimini_array != NULL)
	...
	EXECUTE_ERROR_BLOCK;
	 ...
	 {is_list_element("find_file insert_file", LAST_KBD_COMMAND, ' ')})
     lines = recent_get_files;
   else if (LAST_KBD_COMMAND == switch_buffer_binding)
   ...
   USER_BLOCK0
	...
	matches = lines
	  [where(array_map
		 (Integer_Type, &is_substr, lines, s))];
	if (length(matches))
	  {
	     erase_buffer();
